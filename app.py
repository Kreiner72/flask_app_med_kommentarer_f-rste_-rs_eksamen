...
def get_work_hours(period, user_id):
    ...
    if period == 'day':                                          # Begyngelsen på betinget if/elif/else-statement, der tjekker om 'period' er lig med 'day'
        ...
        start_date = today - timedelta(days=today.weekday())     # vi finder start datoen  ved at tage idags dato og trække antallet af dage der er gået siden ugen startede
        end_date = start_date + timedelta(days=6)                # på samme måde finder vi slut datoen, ved at tage start datoen og plusse med 6. Da start dato altid er mandag (0) og slut dato altid er søndag (6)
    elif period == 'month':                                      # Hvis perioden er "month"
        start_date = today.replace(day=1)                        # Start dato findes ved at tage idag og replacer dagen med 1, som er den første dag i måneden.
        if start_date.month == 12:                               # Hvis start datoen er i december, så virker det trick vi bruger til at finde slut datoen ikke, da første dag i næste måned er et nyt år
            end_date = start_date.replace(day=31)                # Her siger vi bare at slut datoen skal være den 31. december
        else:                                                    # Hvis ikke det er december
            end_date = (start_date.replace(month=start_date.month+1, day=1) - timedelta(days=1)) #Slut datoen bliver 
    else:
        return []                                                #
...
class LoginForm(FlaskForm):         # Oprettelse af klasse: LoginForm-klassen, der nedarver fra FlaskForm-klassen fra flask_wtf modulet
    username = StringField(validators=[InputRequired(), Length(min=3, max=20)], render_kw={"placeholder": "Username"})   # kig i kodebeskrivelsen
    password = PasswordField(validators=[InputRequired(), Length(min=3, max=20)], render_kw={"placeholder": "Password"}) # kig i kodebeskrivelsen
...
@app.route("/delete/<entry_id>", methods=["POST"]) # @app.route("/delete/<entry_id>") = decorator = bruges til routing, fortæller Flask hvilken URL der skal udløse funktionen
@login_required                            #
def db_delete(entry_id):                   # Definere funktionen db_delete() og tager et parameter, entry_id. Denne funktion bruges til at slette en række fra databasen
    ændre_tid(entry_id)                    # Metodekald ændre_tid() med entry_id som argument. Dette sletter en række fra databasen
    flash("Record removed", "danger")      # flash = funktion, der viser en besked til brugeren, 'danger' = en bootstrap klasse, der bruges til at vise en rød besked
    return redirect(url_for("dashboard"))  # Omidirigerer brugeren til dashboard-siden (redirect og url_for = Flask funktioner)

@app.route('/logout')                      # @app.route('/logout') = decorator = bruges til routing, fortæller Flask hvilken URL der skal udløse funktionen
@login_required                            # login_required = decorator = bruges til at sikre, at brugeren er logget ind, før de kan få adgang til logout-siden
def logout():                              # Definere funktionen logout(). Denne funktion bruges til at logge brugeren ud
    logout_user()                          # Metodekald logout_user() på login_manager-objektet. Dette logger brugeren ud
    return redirect(url_for('index'))      # Omidirigerer brugeren til index-siden (redirect og url_for = Flask funktioner)

@app.route('/Skema/<user>')           # @app.route('/Skema') = decorator = bruges til routing, fortæller Flask hvilken URL der skal udløse funktionen
@login_required                       # login_required = decorator = bruges til at sikre, at brugeren er logget ind, før de kan få adgang til skema-siden   
def skema(user):                      # Definere funktionen skema() og tager et parameter, user. Denne funktion bruges til at returnere skema.html-siden
    if user != current_user.username: # if statement = betinget statement, der tjekker om user-objektet er forskellig fra brugerens brugernavn
        flash("You do not have permission to view this page.", "danger") # flash = funktion, der viser en besked til brugeren, 'danger' = en bootstrap klasse, der bruges til at vise en rød besked
        return redirect(url_for("dashboard"))                            # Omidirigerer brugeren til dashboard-siden (redirect og url_for = Flask funktioner)


    with sqlite3.connect('arbejdstider.db') as conn: # Hvis brugeren har tilladelse til at se skemaet, forbindes databasen "arbejdstider.db" og gemmes i variablen conn. with = statement, sqlite3 = modul, connect() = funktion, 'arbejdstider.db' = ARGUEMENT/PARAMETRE?, conn = variabel/"connection"-objekt
        cursor = conn.cursor()                       # Metodekald cursor() på connection-objektet(conn). Dette returnerer et cursor-objekt, som bruges til at udføre SQL-forespørgsler
        cursor.execute("SELECT * FROM tidsændringer WHERE user_id = ?", (current_user.id,)) # Metodekald execute() på cursor-objektet. Ufører en SQL-query, der vælger alle rækker fra "tidsændringer"-tabellen, hvor user_id = current_user.id. '?' = placeholders for udskiftning af parametre og en sikkerhedsmekanisme for at forhindre SQL-injektioner
        tidsændringer_list = cursor.fetchall()       # Metodekald fetchall() på cursor-objektet. Dette returnerer en liste af tuples, der indeholder resultaterne af SQL-forespørgslen

    return render_template("skema.html", tidsændringer_list=tidsændringer_list) # returnere skema.html-siden, der vises i browseren. render_template() = funktion, der bruges til at aflæse html-filer - nedarvet fra Flask-klassen

@app.route('/arbejdstider/<int:user_id>')   # decorator = bruges til routing, fortæller Flask hvilken URL der skal udløse funktionen
@login_required                             # login_required = decorator = bruges til at sikre, at brugeren er logget ind, før de kan få adgang til arbejdstider-siden
def arbejdstider(user_id):                  # funktionen arbejdstider() tager et parameter, user_id. Denne funktion bruges til at returnere arbejdstider.html-siden

    today = datetime.today().date()         # today = variabel, der indeholder dagens dato - udfra: datetime = klasse fra datetime-modul, today() = metodekald på datetime-klasse, date() = metodekald på today-objektet
    today_str = today.strftime("%d-%m-%Y")  # today_str = variabel, der indeholder en streng-værdi af dagens dato, formateret som "dag-måned-år"

    start_of_week = today - timedelta(days=today.weekday()) # start_of_week = variabel, der indeholder starten af ugen, beregnet ved at tage dagens dato og trække antallet af dage, der er gået siden ugen startede
    end_of_week = start_of_week + timedelta(days=6)         # end_of_week = variabel, der indeholder slutningen af ugen, beregnet ved at tage starten af ugen og plusse med 6

    daily_hours = get_work_hours('day', user_id)     # daily_hours = variabel, der indeholder resultatet af get_work_hours() funktionen, der henter arbejdstider for den bruger, der er logget ind
    weekly_hours = get_work_hours('week', user_id)   # weekly_hours = variabel, der indeholder resultatet af get_work_hours() funktionen, der henter arbejdstider for den bruger, der er logget ind
    monthly_hours = get_work_hours('month', user_id) # monthly_hours = variabel, der indeholder resultatet af get_work_hours() funktionen, der henter arbejdstider for den bruger, der er logget ind
#
    daily_hours_today = [entry for entry in daily_hours if entry[1] == today_str]  # daily_hours_today = variabel, der indeholder en liste af arbejdstider for den bruger, der er logget ind, hvor datoen er lig med dagens dato
#
    weekly_hours_this_week = [entry for entry in weekly_hours if start_of_week <= datetime.strptime(entry[1], "%d-%m-%Y").date() <= end_of_week] 
# weekly_hours_this_week = variabel, der indeholder en liste af arbejdstider for den bruger, der er logget ind, hvor datoen er mellem starten og slutningen af ugen
...
# returnere work_hours.html-siden, der vises i browseren. render_template() = funktion, der bruges til at aflæse html-filer - nedarvet fra Flask-klassen
if __name__ == "__main__":               #  __name__ = special prædefineret variabel, som er brugt af Python "tolken" til at identificere navnet på det aktuelle script
    app.run(host="0.0.0.0", debug=True)  # Hvis __name__ er lig med "__main__", så kører applikationen. app.run() = metodekald, der starter Flask-applikationen. host="
